...
export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<WalletGetReferralsResponseDto | ErrorResponseDto>
) {
  try {
    if (req.method !== 'GET') {
      return res
    ...
    if (wallet?._id) {
      const wallets = await database
        .collection<WalletDto>(WALLET_COLLECTION)
        .find({
          referralBy: wallet.publicAddress,
        })
        .toArray();

      return res.status(HTTP_SUCCESS).json({
        sumOfReferrals: wallets.length,
      });
    } else {
      return res.status(HTTP_SUCCESS).json({
